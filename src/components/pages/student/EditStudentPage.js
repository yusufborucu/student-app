...
export default class EditStudentPage extends Component {
    ...
    componentDidMount() {
        axios.get(`${API_BASE}/student/` + this.props.match.params.id)
            .then(response => {
                let classrooms = [];
                let obj = {id: 0, name: "Seçiniz"};
                classrooms.push(obj);
                classrooms = classrooms.concat(response.data.classrooms);
                this.setState({
    ...
    onSubmit(e) {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            axios.put(`${API_BASE}/student/` + this.props.match.params.id, obj)
                .then(response => {
